...
-- Which waiters have taken 2 or more bills on a single date? List names, dates, and bill counts
...
-- Rooms with tables that can serve more than 6 people and how many such tables they have
SELECT rt.room_name, COUNT(*) AS large_tables
FROM restRest_table rt
WHERE rt.no_of_seats > 6
GROUP BY rt.room_name;

-- Rooms and the total amount of bills in each room
SELECT rt.room_name, SUM(b.bill_total) AS total_bills
FROM restBill b
JOIN restRest_table rt ON b.table_no = rt.table_no
GROUP BY rt.room_name;

-- Headwaiters' names and total bill amounts their waiters have taken (largest first)
SELECT hs.first_name, hs.surname, SUM(b.bill_total) AS total_bills
FROM restStaff hs
...
-- Which waiters have taken 3 or more bills on a single date? List names and bill counts
SELECT s.first_name, s.surname, b.bill_date, COUNT(*) AS bill_count
FROM restBill b
JOIN restStaff s ON b.waiter_no = s.staff_no
GROUP BY s.first_name, s.surname, b.bill_date
