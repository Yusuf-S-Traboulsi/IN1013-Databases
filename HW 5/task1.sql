...
-- List the names of all customers whose last name is Smith, each only once in reverse lexicographical order
SELECT DISTINCT cust_name
FROM restBill
WHERE cust_name LIKE '%Smith'
ORDER BY cust_name DESC;

-- List the names of all customers whose second names begin with 'C', each only once
SELECT DISTINCT cust_name
FROM restBill
...
-- List the dates the restaurant took money in 2015, each date only once in sorted order
